WSM Content Assistant
An intelligent, conversational AI assistant designed to help users find and understand software documentation. This application leverages the Google Gemini API to provide natural language search and on-demand document summarization.


✨ Features
 - Conversational Search: Ask for documents in plain English (e.g., "find me case studies for ADManager Plus").
 - Dynamic Document Summarization: Get a concise summary of any public-facing document (HTML or PDF) with a single click.
 - Sleek, Modern UI: A custom-themed, responsive user interface designed for a great user experience.
 - Robust Backend: Built with Python and Flask, using SQLAlchemy for database interaction.

🛠️ Tech Stack
 - Backend: Python, Flask
 - AI: Google Generative AI (Gemini 1.5 Flash)
 - Database: PostgreSQL (via SQLAlchemy)
 - Frontend: HTML5, CSS3, JavaScript (ES6)
 - Deployment: Gunicorn (for production)

📖 Libraries:

 - requests for HTTP requests
 - BeautifulSoup4 & PyMuPDF for web and PDF content extraction
 - python-dotenv for environment variable management
